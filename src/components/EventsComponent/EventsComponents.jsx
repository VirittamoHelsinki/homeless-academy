  ...
  return (
    <div className="pt-4">
      ...
      <div
        ...
        id="eventsSection"
        className="pt-8 pl-8 pr-8 pb-6 lg:pl-16 lg:pr-16"
      >
        <div className="grid grid-cols-1 lg:gap-y-4 lg:grid-cols-3 lg:gap-4 p-4 w-full">
          {currentEvents.map((event, index) => (
            <div
              key={index}
              className="card shadow-lg lg:shadow-sm items-left pb-3 ps-6 mb-2"
            >
              <div className="flex items-left gap-x-8 pt-6 lg:pt-10">
                <h2 className="font-lexend font-bold">{event?.dates}</h2>
              ...
              <p className="font-lexend text-sm text-base py-2 lg:w-11/12 pb-8 pr-3 text-justify">
                {event?.description}
              </p>
            </div>
          ))}
        ...
        {filteredEvents.length > 9 && (
          <Pagination
            scrollToEventsSection={scrollToEventsSection}
            articlesPerPage={eventsPerPage}
            totalArticles={filteredEvents.length}
            paginate={paginate}
