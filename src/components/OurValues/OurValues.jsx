...
const OurValues = () => {
  ...
  return (
    <div className='bg-dark-green pb-8 pt-4'>
      <h1 className='pt-8 lg:ml-16 text-white  lg:pb-5 font-sans text-2xl lg:text-4xl font-bold pt-10 text-center lg:text-left'> {language === 'fi-FI' ? 'Tärkeimmät arvomme' : 'Our key values'}</h1>
      <div className='lg:grid lg:grid-cols-3 pt-8 lg:ml-16  gap-y-10 lg:gap-y-4 gap-x-24 lg:gap-x-2'>
        {values.map((value, index) => (
          ...
          >
            <img className='w-14 h-14' src={value?.icone?.fields?.file?.url} alt='' />
